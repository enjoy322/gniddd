...
app.get("/oil/:vin", async (req, res) => {
  try {
    ...
    if (!url) {
      ...
      return res.json({ car, url: null, source: gpt?.found ? "gpt_global" : "not_found", oil, recommendations: buildRecommendations(oil) });
    }
    ...
    if (engine) {
      ...
      return res.json({ car, url, source: "parsed", oil: engine, recommendations: buildRecommendations(engine) });
    }
    ...
    if (gptPage?.found) {
      ...
      return res.json({ car, url, source: "gpt_html", oil, recommendations: buildRecommendations(oil) });
    }
    ...
    if (gptGlobal?.found) {
      ...
      return res.json({ car, url, source: "gpt_global", oil, recommendations: buildRecommendations(oil) });
    }
